...
export async function POST(req:Request,{params}:Ctx){
 ...
 try{
  ...
  const path=id;
  const {error:insertError}=await client.from(theme.tables.reports).insert({id,user_id:user.id,role,nowgo_store_id:role==='owner'?storeId:null,phone,business_number:role==='owner'?businessNumber:null,place_id:placeId,menu_id:id,shop,menu,address,price,heat,flavor,category,observed_at:observed,note,lat,lng,photo_path:path,photo_mime:mime,status:'draft'});
  ...
  const {error:uploadError}=await client.storage.from(theme.bucket).upload(path,bytes,{contentType:mime,upsert:false});
