...
import {menusFor} from '@/lib/menus';
import {communityMenus,menuById} from '@/lib/menu-catalog';
import {verifiedUser} from '@/lib/supabase';
import {EXPERIENCES,experienceFromRequest} from '@/lib/experience';
...
export async function GET(req:Request,{params}:Ctx){
 ...
 try{
  ...
  if(action==='menus'){const url=new URL(req.url),num=(name:string)=>Number(url.searchParams.get(name)||0);return reply(req,await communityMenus(key,{page:num('page'),query:url.searchParams.get('q')||'',heat:num('heat'),maxHeat:num('maxHeat'),flavor:url.searchParams.get('flavor')||'',category:url.searchParams.get('category')||'',budget:num('budget')}))}
  if(action==='menu'){const id=new URL(req.url).searchParams.get('id')||'';return reply(req,{menu:await menuById(id,key)})}
  ...
  const [saves,reports]=await Promise.all([
   auth.client.from('hot_menu_saves').select('menu_id').eq('user_id',auth.user.id).eq('experience_key',key).limit(100),
   auth.client.from('hot_taste_observations').select('id,menu,status,created_at').eq('user_id',auth.user.id).eq('experience_key',key).order('created_at',{ascending:false}).limit(20)
  ]);
...
export async function POST(req:Request,{params}:Ctx){
 ...
 const {action}=await params;
 const key=experienceFromRequest(req),profile=EXPERIENCES[key];
 try{
  ...
  if(action==='state'){
   ...
   if(typeof p.saved!=='boolean'||!await menuById(p.menuId,key))return reject(req,'메뉴를 다시 선택해 주세요.',400);
   const q=p.saved?client.from('hot_menu_saves').upsert({user_id:user.id,experience_key:key,menu_id:p.menuId},{onConflict:'user_id,experience_key,menu_id'}):client.from('hot_menu_saves').delete().eq('user_id',user.id).eq('experience_key',key).eq('menu_id',p.menuId);
  ...
  if(!/^[a-f0-9-]{36}$/.test(id)||!['customer','owner'].includes(role)||shop.length<2||shop.length>100||menu.length<2||menu.length>100||menusFor(key).some(x=>x.shop===shop)||!isKoreanAddress(address)||!Number.isInteger(price)||price<100||price>1000000||!Number.isInteger(heat)||heat<1||heat>5||!profile.flavors.slice(1).includes(flavor)||note.length>1000||!/^\d{4}-\d{2}-\d{2}$/.test(observed)||!Number.isFinite(Date.parse(observed))||new Date(observed)>new Date()||new Date(observed).toISOString().slice(0,10)!==observed||get('rights')!=='yes'||get('accuracy')!=='yes'||!profile.categories.includes(category))return reject(req,'대한민국 내 실제 가게 주소와 메뉴·가격·확인 날짜·사진 공개 동의를 확인해 주세요.',400);
  ...
  const path=id;
  const {error:insertError}=await client.from('hot_taste_observations').insert({id,user_id:user.id,experience_key:key,role,nowgo_store_id:role==='owner'?storeId:null,phone,business_number:role==='owner'?businessNumber:null,place_id:placeId,menu_id:id,shop,menu,address,price,heat,flavor,category,observed_at:observed,note,lat,lng,photo_path:path,photo_mime:mime,status:'draft'});
...
export async function DELETE(req:Request,{params}:Ctx){
 ...
 try{
  ...
  const {data,error}=await auth.client.from('hot_taste_observations').select('photo_path').eq('id',id).eq('user_id',auth.user.id).eq('experience_key',key).maybeSingle();
  if(error)throw error;if(!data)return reply(req,{deleted:true});
  const {error:deleteError}=await auth.client.from('hot_taste_observations').delete().eq('id',id).eq('user_id',auth.user.id).eq('experience_key',key);
