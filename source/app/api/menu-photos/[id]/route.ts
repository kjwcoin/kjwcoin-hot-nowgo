...
export async function GET(_req:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  const db=publicDb();const {data,error}=await db.from('sweet_public_menus').select('id').eq('id',id).maybeSingle();
  if(error||!data)return new Response('Not found',{status:404});
  const {data:file,error:storageError}=await db.storage.from('sweet-report-photos').download(id);
