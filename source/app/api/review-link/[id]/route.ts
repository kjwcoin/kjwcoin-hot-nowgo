...
export async function GET(req:Request,{params}:{params:Promise<{id:string}>}){
 try{
  const variant=variantForHost(req.headers.get('host')),{id}=await params,menu=await menuByPlaceId(id,variant);
  if(!menu)return reply(req,{url:null},404);
  if(menu.isDemo)return reply(req,{url:null});
  if(menu.verifiedOwner&&menu.nowgoSlug&&/^[a-zA-Z0-9_-]{1,100}$/.test(menu.nowgoSlug))return reply(req,{url:`https://www.nowgo.space/p/${menu.nowgoSlug}?source=${variant}`});
