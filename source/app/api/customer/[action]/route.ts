...
export async function GET(req:Request,{params}:Ctx){
 try{
  ...
  const {data:consent,error}=await auth.client.from(theme.tables.consents).select('essential_version').eq('user_id',auth.user.id).maybeSingle();
  if(error)throw error;
  if(consent?.essential_version!==theme.consent)return reply(req,{customer:null,auth:authConfig,consentRequired:true});
...
export async function POST(req:Request,{params}:Ctx){
 ...
 try{
  const theme=siteConfig(variantForHost(req.headers.get('host')));
  if(action!=='consents')return reply(req,{error:'NOWGO 통합회원으로 가입합니다.'},410);
  ...
  if(body.essential!==true||body.version!==theme.consent||typeof body.marketingEmail!=='boolean')return reply(req,{error:'필수 동의를 확인해 주세요.'},400);
  const now=new Date().toISOString();
  const {error}=await auth.client.from(theme.tables.consents).upsert({
   user_id:auth.user.id,essential_version:theme.consent,essential_at:now,
