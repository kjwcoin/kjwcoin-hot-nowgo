...
export async function GET(req:Request,{params}:Ctx){
 try{
  ...
  if(action==='owned-stores'){
   ...
   const {data,error}=await auth.client.rpc('rich_owned_stores');if(error)throw error;
  ...
  const {data:consent,error}=await auth.client.from('rich_member_consents').select('essential_version').eq('user_id',auth.user.id).maybeSingle();
...
export async function POST(req:Request,{params}:Ctx){
 ...
 try{
  if(action!=='consents')return reply(req,{error:'RICH은 NOWGO 통합회원으로 가입합니다.'},410);
  ...
  const {error}=await auth.client.from('rich_member_consents').upsert({
