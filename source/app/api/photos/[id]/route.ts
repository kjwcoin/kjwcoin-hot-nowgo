...
export async function GET(req:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  const {data,error}=await auth.client.from('hot_taste_observations').select('photo_path').eq('id',id).eq('user_id',auth.user.id).eq('experience_key',experienceFromRequest(req)).maybeSingle();
