...
export default function Join(){
 ...
 async function google(){
  ...
  try{
   sessionStorage.setItem('sweet-pending-consent',JSON.stringify({essential:true,marketingEmail:marketing,returnTo}));
 ...
 return <main className="terms-page join-page"><Brand/><a href="/" className="text-link">← 메뉴 지도로</a><div className="eyebrow">NOWGO UNIFIED ACCOUNT</div><h1>한 번 가입하고<br/>디저트를 알려요.</h1><p>SWEET에서 가입해도 NOWGO와 같은 계정을 사용합니다. 이미 NOWGO 회원이라면 같은 구글 계정으로 계속하세요.</p>
  <section className="join-card"><h2>가입 전에 확인해 주세요</h2><p>SWEET은 회원 ID, 저장·제보 이력, 제보할 때 적은 연락처·사업자번호와 사진을 제보 운영 및 문의에 사용합니다. 연락처와 사업자번호는 공개하지 않습니다. 회원 탈퇴나 해당 제보 삭제 시 삭제하며 법령상 보존 의무가 있는 기록은 해당 기간에만 분리 보관합니다. 필수 이용에 동의하지 않아도 지도는 볼 수 있지만 제보·저장은 이용할 수 없습니다.</p>
  <label className="consent"><input type="checkbox" checked={essential} onChange={e=>setEssential(e.target.checked)}/><span>[필수] <a href="/terms" target="_blank" rel="noreferrer">SWEET 이용·개인정보 안내</a> 및 <a href="https://www.nowgo.space/legal/privacy" target="_blank" rel="noreferrer">NOWGO 개인정보 처리방침</a>을 확인하고 가입에 동의합니다.</span></label>
  ...
  {signedIn?<button className="submit-button" onClick={finish} disabled={busy}>{busy?'연결하는 중':'이 계정으로 SWEET 이용하기 ↗'}</button>:<button className="submit-button" onClick={google} disabled={busy}>{busy?'연결하는 중':'구글로 통합회원 가입·로그인 ↗'}</button>}
