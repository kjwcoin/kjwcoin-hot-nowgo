...
export default function Join(){
 ...
 return <main className="terms-page join-page"><Brand/><a href="/" className="text-link">← 메뉴 지도로</a><div className="eyebrow">NOWGO UNIFIED ACCOUNT</div><h1>한 번 가입하고<br/>한 접시를 알려요.</h1><p>{theme.name}에서 가입해도 NOWGO와 같은 계정을 사용합니다. 이미 NOWGO 회원이라면 같은 구글 계정으로 계속하세요.</p>
  ...
  {signedIn?<button className="submit-button" onClick={finish} disabled={busy}>{busy?'연결하는 중':'이 계정으로 {theme.name} 이용하기 ↗'}</button>:<button className="submit-button" onClick={google} disabled={busy}>{busy?'연결하는 중':'구글로 통합회원 가입·로그인 ↗'}</button>}
