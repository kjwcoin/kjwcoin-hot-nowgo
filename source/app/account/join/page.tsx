...
export default function Join(){
 const [returnTo,setReturnTo]=useState('/map'),[signedIn,setSignedIn]=useState(false),[essential,setEssential]=useState(false),[marketing,setMarketing]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState('');
 ...
 return <main className="terms-page join-page"><Brand/><a href="/map" className="text-link">← 메뉴 지도로</a><div className="eyebrow">NOWGO UNIFIED ACCOUNT</div><h1>한 번 가입하고<br/>한 접시를 알려요.</h1><p>HOT에서 가입해도 NOWGO와 같은 계정을 사용합니다. 이미 NOWGO 회원이라면 같은 구글 계정으로 계속하세요.</p>
