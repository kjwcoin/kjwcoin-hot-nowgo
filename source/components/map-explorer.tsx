...
export default function MapExplorer({variant='hot'}:{variant?:SiteVariant}){
 ...
 const locatedCatalog=useMemo(()=>menusWithinRadius(catalog,userLocation,15,demoLocations&&demoLocations.origin.lat===userLocation?.lat&&demoLocations.origin.lng===userLocation?.lng?demoLocations.points:[]),[catalog,userLocation,demoLocations]);
 ...
 return <main className="explorer">
  ...
  <div className="explorer-surface">
   ...
   <aside className={`explorer-results ${expanded?'expanded':'collapsed'}`} aria-label="검색된 메뉴">
    ...
    <button className="explorer-results-heading" aria-expanded={expanded} aria-controls="map-results" onClick={()=>setExpanded(!expanded)}><span><small>{userLocation?'내 위치 기준 15km':'위치 확인 후 주변 메뉴 표시'}</small><strong>지금 당기는 한 접시 <b>{filtered.length}</b></strong></span>{expanded?<ChevronDown size={20}/>:<ChevronUp size={20}/>}</button>
     <a className="explorer-report explorer-report--inline" href="/suggestion#report"><Plus size={16}/><span>제보 및 등록</span></a>
    </div>
    {expanded&&<div id="map-results" className="explorer-results-body"><p className="explorer-example">{userLocation?'예시 가매장은 주소가 확인된 위치에만 표시 · 실제 영업하지 않아요':'위치 권한을 허용하면 15km 안의 메뉴가 자동으로 표시돼요'}</p>
