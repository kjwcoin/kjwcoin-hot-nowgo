...
import {menusFor,money,type Menu} from '@/lib/menus';
import {EXPERIENCES,type ExperienceKey} from '@/lib/experience';
import {api,track} from '@/lib/client';
export default function MapExplorer({experience='hot'}:{experience?:ExperienceKey}){
 const profile=EXPERIENCES[experience],MENUS=useMemo(()=>menusFor(experience),[experience]),HEAT=profile.intensity,FLAVORS=profile.flavors;
 ...
 useEffect(()=>{let cancelled=false;const p=parameters(0);const refresh=()=>api<{menus:Menu[];hasMore:boolean}>(`/api/menus?${p}`).then(d=>{if(!cancelled){setCatalog([...MENUS,...d.menus]);setPage(0);setHasMore(d.hasMore)}}).catch(()=>{});const timer=setTimeout(refresh,query?250:0);window.addEventListener('focus',refresh);return()=>{cancelled=true;clearTimeout(timer);window.removeEventListener('focus',refresh)}},[query,heat,flavor,category,budget]);
 ...
 return <main className="explorer" data-experience={experience}>
  <header className="explorer-toolbar">
   <div className="explorer-topline"><Brand experience={experience}/><div className="explorer-search"><Search size={21}/><Input aria-label={`${profile.title} 검색`} value={query} onChange={e=>setQuery(e.target.value)} placeholder={profile.searchPlaceholder}/>{query&&<Button variant="ghost" size="icon" aria-label="검색어 지우기" onClick={()=>setQuery('')}><X size={18}/></Button>}</div><span className="explorer-region"><MapPin size={16}/>대한민국 전국 <small>{profile.name}</small><a className="explorer-promo" href="/suggestion">{profile.name} 소개 ↗</a></span><CustomerPanel experience={experience}/></div>
   <nav className="experience-switch" aria-label="맛 지도 선택"><a href="https://www.nowgo.space/">NOWGO 전체 서비스</a>{(['hot','rich','sweet'] as const).map(key=><a key={key} href={`https://${EXPERIENCES[key].host}/`} aria-current={key===experience?'page':undefined}>{EXPERIENCES[key].name}</a>)}</nav>
   <div className="explorer-filters" aria-label="메뉴 검색 조건">
    <label>{profile.intensityLabel}<NativeSelect aria-label={profile.intensityLabel} value={heat} onChange={e=>setHeat(Number(e.target.value))}><option value={0}>모든 단계</option>{HEAT.map((h,i)=><option key={h} value={i+1}>{i+1}단계 · {h}</option>)}</NativeSelect></label>
    <label>{experience==='sweet'?'스윗한 맛':experience==='rich'?'느끼한 맛의 결':'매운맛 카테고리'}<NativeSelect aria-label="맛의 결" value={flavor} onChange={e=>setFlavor(e.target.value)}>{FLAVORS.map(f=><option key={f} value={f}>{f==='전체'?'모든 맛의 결':f}</option>)}</NativeSelect></label>
    <label>{experience==='sweet'?'스윗한 디저트 메뉴':experience==='rich'?'느끼한맛 메뉴':'매운맛 메뉴'}<NativeSelect aria-label="메뉴 종류" value={category} onChange={e=>setCategory(e.target.value)}><option value="전체">모든 메뉴</option>{profile.categories.map(c=><option key={c} value={c}>{c}</option>)}</NativeSelect></label>
