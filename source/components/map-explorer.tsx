 ...
 return <main className="explorer">
  ...
  <div className="explorer-surface">
   ...
   <aside className={`explorer-results ${expanded?'expanded':'collapsed'}`} aria-label="검색된 메뉴">
    <button className="explorer-results-heading" aria-expanded={expanded} aria-controls="map-results" onClick={()=>setExpanded(!expanded)}><span><small>{userLocation?'내 위치 반경 30km':'전국의 느끼한 맛'}</small><strong>지금 당기는 리치 메뉴 <b>{filtered.length}</b></strong></span>{expanded?<ChevronDown size={20}/>:<ChevronUp size={20}/>}</button>
