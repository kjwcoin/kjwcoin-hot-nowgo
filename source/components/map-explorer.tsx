...
import {menusWithinRadius,type GeoPoint} from '@/lib/nearby-demo';
...
export default function MapExplorer({variant='hot'}:{variant?:SiteVariant}){
 ...
 const locatedCatalog=useMemo(()=>menusWithinRadius(catalog,userLocation,15),[catalog,userLocation]);
 ...
 return <main className="explorer">
  <header className="explorer-toolbar">
   <div className="explorer-topline"><Brand/><div className="explorer-search"><Search size={21}/><Input aria-label={`${theme.name} 메뉴 검색`} value={query} onChange={e=>setQuery(e.target.value)} placeholder={theme.search}/>{query&&<Button variant="ghost" size="icon" aria-label="검색어 지우기" onClick={()=>setQuery('')}><X size={18}/></Button>}</div><span className="explorer-region"><MapPin size={16}/>내 위치 15km <small>{theme.name}</small><a className="explorer-promo" href="/suggestion">{theme.name} 소개 ↗</a></span><CustomerPanel variant={variant}/></div>
  ...
  <div className="explorer-surface">
   <KakaoMap fullScreen menus={filtered} selectedId={current?.id} onSelect={select} onLocation={point=>{setSelected(null);setUserLocation(point)}} variant={variant}/>
   <aside className={`explorer-results ${expanded?'expanded':'collapsed'}`} aria-label="검색된 메뉴">
    <div className="explorer-results-header">
    <button className="explorer-results-heading" aria-expanded={expanded} aria-controls="map-results" onClick={()=>setExpanded(!expanded)}><span><small>{userLocation?'내 위치 기준 15km':'내 위치 버튼을 눌러 주세요'}</small><strong>지금 당기는 한 접시 <b>{filtered.length}</b></strong></span>{expanded?<ChevronDown size={20}/>:<ChevronUp size={20}/>}</button>
     <a className="explorer-report explorer-report--inline" href="/suggestion#report"><Plus size={16}/><span>제보 및 등록</span></a>
    </div>
    {expanded&&<div id="map-results" className="explorer-results-body"><p className="explorer-example">{userLocation?'15km 안의 가매장 3곳은 가상 위치 · 실제 영업하지 않아요':'내 위치 버튼을 누르면 15km 안의 메뉴를 보여드려요'}</p>
