...
export default function KakaoMap({menus,onSelect,onPoint,onLocation,fullScreen=false,selectedId,variant='hot'}:Props){
 ...
 const fitNearby=useCallback((point:GeoPoint)=>{
  ...
  const longitudeDelta=10/(111.32*Math.max(.2,Math.cos(point.lat*Math.PI/180)));
 ...
 const statusText=locationState==='located'?'내 위치 기준 30km · 가상 매장 3곳':locationState==='locating'?'위치 확인 중':locationState==='denied'?'위치 권한 필요':locationState==='inaccurate'?'정확한 위치 확인 필요':'GPS 기준 30km만 표시';
 return <div className={fullScreen?'map-panel map-fullscreen':'map-panel'} ref={rootRef}>
  ...
  {mapState==='ready'&&locationState!=='located'&&<div className="map-location-gate" role="status" aria-live="polite"><LocateFixed size={28}/><h3>{locationState==='locating'?'내 위치를 확인하고 있어요':'위치 권한이 필요해요'}</h3><p>{locationState==='inaccurate'?'정확한 위치를 확인할 수 없어요. 기기의 위치 설정을 켜고 다시 시도해 주세요.':locationState==='denied'?'위치 권한을 허용해야 현재 위치에서 30km 안의 매장만 볼 수 있어요.':'GPS 확인 후 30km 안의 매장만 지도에 표시합니다.'}</p></div>}
