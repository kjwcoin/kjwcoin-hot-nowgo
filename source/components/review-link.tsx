...
export default function ReviewLink({placeId,isDemo}:{placeId:string,isDemo:boolean}){
 ...
 useEffect(()=>{let active=true;setUrl(null);if(!isDemo)api<{url:string|null}>('/api/review-link/'+encodeURIComponent(placeId)).then(d=>{if(active)setUrl(d.url)}).catch(()=>{});return()=>{active=false}},[placeId,isDemo]);
