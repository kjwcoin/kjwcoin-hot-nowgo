...
대한민국 전국의 매운 메뉴를 발견하고 통합회원이 제보하는 사이트입니다. 메뉴 지도 `/`, 프로모션 `/suggestion`, HOT 통합회원 가입 `/account/join`, 제보 `/suggestion#report`를 제공합니다. 지도와 제보 주소는 전국을 지원하고, 화면의 가매장 3곳은 실제 영업하지 않는 예시입니다. 상단에는 빨간 NOWGO 이미지 로고만 표시하고 컨셉 문구는 프로모션 본문에 둡니다.
